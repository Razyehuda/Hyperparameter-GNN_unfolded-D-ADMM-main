...
class GNNHypernetworkNew(nn.Module):
    def __init__(self, P, m, hidden_dim):
        super(GNNHypernetworkNew, self).__init__()
        self.P = P
        self.m = m
...
class DLASSO_GNNHypNew(nn.Module):
    def __init__(self, A, args):
        ...
        # Improved encoder
        self.encoder = GNNHypernetworkNew(P=self.P,
                                         m=self.n * 2,
        ...
        # Parameter bounds with better scaling - ensure they're on the same device as A
        self.alpha_max = torch.tensor(args.alpha_max, device=A.device)
        self.tau_max = torch.tensor(args.tau_max, device=A.device)
        self.rho_max = torch.tensor(args.rho_max, device=A.device)
        self.eta_max = torch.tensor(args.eta_max, device=A.device)
    ...
    def forward(self, b, graph_list):
        ...
        sum_neighbors = self.compute_sum_neighbors(graph_list, device=b.device)
        ...
        for k in range(K):
            ...
            max_grad_norm = 1  # Reduce clipping threshold over iterations
            ...
            delta = self.compute_delta(graph_list, y_next, device=b.device)
    ...
    def compute_sum_neighbors(self, graph_list, device=None):
        batch_size = len(graph_list)
        sum_neighbors = torch.zeros((batch_size, self.P, 1, 1), device=device if device is not None else self.A.device)
    ...
    def compute_delta(self, graph_list, y1, y2=None, device=None):
        if y2 is None:
            y2 = y1
        delta = torch.zeros_like(y1, device=device if device is not None else y1.device)
        batch_size = len(graph_list)
