...
class DLASSO_GNNHyp3(nn.Module):
    def __init__(self, A, args):
        ...
        # Initialize bias to output reasonable default hyperparameters
        # This helps the model start with good values instead of random ones
        with torch.no_grad():
            # Set bias to output values around 0.5 after sigmoid
            # This gives reasonable starting hyperparameters
            self.fc.bias.data[0] = 0.0  # alpha (will be scaled by alpha_max)
            self.fc.bias.data[1] = 0.0  # tau (will be scaled by tau_max) 
            self.fc.bias.data[2] = 0.0  # rho (will be scaled by rho_max)
            self.fc.bias.data[3] = 0.0  # eta (will be scaled by eta_max)
    ...
    def forward(self, b, graph_list):
        ...
        for k in range(K):
            ...
            tau_k = torch.clamp(tau_k, max=0.98)     # Almost no limit on tau
            rho_k = torch.clamp(rho_k, max=0.98)     # Almost no limit on rho
            eta_k = torch.clamp(eta_k, max=0.98)     # Almost no limit on eta
